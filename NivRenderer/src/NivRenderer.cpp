...
{
    // Create Folder "New Project" in current working folder (Marked as Temporary location)
    auto test = std::filesystem::remove_all(Project::DEFAULT_FOLDER);
    std::filesystem::create_directory(Project::DEFAULT_FOLDER);
    m_Project = CreateScope<Project>(Project::DEFAULT_PATH);
    ...
    setupDefaultScene();
    SerializationManager::SaveProject(m_Project.get(), true, true);
...
void Application::handleWindowCommand(WindowCommandEvent command)
{
    ...
    if (command.GetCommand() == WindowCommand::SaveProject)
        SerializationManager::SaveProject(m_Project.get(), true, false);
    if (command.GetCommand() == WindowCommand::SaveProjectAs)
        SerializationManager::SaveProject(m_Project.get(), false, false);
    ...
    // TODO: Implement "New Project": Ask for Location and create New Project file in the given folder (If file already exists ask to overwrite it)
}
