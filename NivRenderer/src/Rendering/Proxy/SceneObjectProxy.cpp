...
void SceneObjectProxy::SetTransform(glm::vec3 position, glm::vec3 scale, glm::vec3 rotation)
{
    ...
    const glm::mat4 rotationX = glm::rotate(glm::mat4(1.0f), rotation.x, glm::vec3(1.0f, 0.0f, 0.0f));
    const glm::mat4 rotationY = glm::rotate(glm::mat4(1.0f), rotation.y, glm::vec3(0.0f, 1.0f, 0.0f));
    const glm::mat4 rotationZ = glm::rotate(glm::mat4(1.0f), rotation.z, glm::vec3(0.0f, 0.0f, 1.0f));
    const glm::mat4 rotationMatrix = rotationY * rotationX * rotationZ;

    m_ModelMatrix = glm::translate(m_ModelMatrix, position) * rotationMatrix * glm::scale(m_ModelMatrix, scale);
}
